...
def Rid(grade):
    ...
    for row in rows:
        tSet.add(row[0])
    return list( tSet ) 
def lDay( beginDate, endDate, holiday, workday ):
    ...
    for hi in hisy:
        if hi < int(ny):
            ...
            logger.info(wk)
            #pdb.set_trace()
            #进行下面语句之前，数据库中已经建好一个临时数据表AttttLdate
            sql = """insert into AtttLdate(lenddate, backdate, returndate) \
                    select lenddate + ?, backdate + ?, returndate + ? from Abklendwork\
                    where lenddate + ? > ? and backdate + ? < ?;\
                    insert into AtttLdate(lenddate, backdate, returndate) \
