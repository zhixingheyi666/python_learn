...
def main():
    ...
    cnv1.create_window(387,15,window=ttk.Button(root,text=u'发送',width=12,\
            command=lambda: print( 'Enty.get()', Enty.get(),\
            TQueue.put( Enty.get() ) ) ) )\
            ##TQueue.put(Enty.get())))
    cnv1.create_window(472,15,window=ttk.Button(root,text=u'清除',width= 9,command=lambda:txt1.delete('1.0','end')))
