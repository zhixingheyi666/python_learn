...
def rest():
    print('经过5分钟休息，您现在精力充沛啦！')

def inimage():
    txgr.image_create( END, image = photog1 )
...
txgr = Text( group, height = 6, width = 60 )
txgr.grid( row = 1, column = 2, rowspan = 2, columnspan = 2 )
txgr.insert( INSERT, '一张一弛')
txgr.insert( END, '是为道也')

txb1 = Button( txgr, text = '休息一下吧！', command = inimage )
txgr.window_create( INSERT, window = txb1 )
